...
import datasets
import torch
from build_dataset import build_instruction_dataset, DataCollatorForSupervisedDataset, build_pre_train_dataset
import transformers
...
def main():
    ...
    if training_args.do_train:
        with training_args.main_process_first(desc="loading and tokenization"):
            path = data_args.dataset_dir
            # files = [os.path.join(path,file.name) for file in path.glob("*.json")]
            logger.info(f"Training files: {path}")
            if "tweet" in path:
                train_dataset = build_pre_train_dataset(
                    data_path=path,
                    tokenizer=tokenizer,
    ...
    if training_args.do_eval:
        with training_args.main_process_first(desc="loading and tokenization"):
            ...
            if "movie" in path:
                ...
                eval_dataset = build_pre_train_dataset(
                    data_path=files,
                    tokenizer=tokenizer,
                    max_seq_length=data_args.max_seq_length,
                    data_cache_dir = None,
    ...
    if training_args.peft_path is not None: # --------------------------> train from the trained lora model
        ...
        with open(training_args.peft_path+"/adapter_config.json","r",encoding="utf-8") as f:
            peft_config = json.load(f)
        if training_args.setting != peft_config["setting"]:
            peft_config["setting"] = training_args.setting
        with open(training_args.peft_path+"/adapter_config.json","w",encoding="utf-8") as f:
        ...
        lora_setting = training_args.setting
